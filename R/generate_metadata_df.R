...
#'
#' @description
#' Generate an "empty" dataframe with the required filename and samplename columns to include in a SummarizedExperiment.
#'
#' Names in the samplename column can be modified, as long as each sample has an unique name.
#'
#' __The filename column cannot be modified, unless the filenames on disk have changed.__ Otherwise, it would prevent \code{\link{create_se}} from linking data and metadata...
#'
#'
#' @md
...
generate_metadata_df <- function(counts_folder_path,
  ...
  check_type(counts_folder_path, "character", "counts_folder_path", length = 1)
  check_type(create_samplename_col, "logical", "create_samplename_col", length = 1)
