...
#' (internal) Compute and plot Relative Log Expression for a chosen data in as SummarizedExperiment
#'
#' @param mat matrix
...
.plot_rle <- function(mat = NULL, show_outlier, col_to_plot) {
  ...
  rle.plot <- ggplot2::ggplot(rle_calc, ggplot2::aes(x = key, value)) +
    ggplot2::geom_boxplot(
      outlier.shape = outlier_shape, outlier.size = 0.3,
      fill = ifelse(rle_grouped$median < mad, "#D55E00", "white")
    ) + # Palette Vermilion
...
#' Relative Log Expression plot of a SummarizedExperiment object’s counts.
#'
