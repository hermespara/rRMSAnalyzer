...
SummarizedExperiment Name : **`r params$ribo_name`**. <br> Number of uploaded samples: **`r ncol(ribo_to_check)`** samples. <br> RNA(s) used by the coverage tool: **`r as.character(SummarizedExperiment::metadata(ribo_to_check)[["rna_names"]][["original_name"]])`** <br> There are **`r as.character(all_sites)`** genomic positions.
<br>
<br>
__**C-score**__ <br>
Method of computation : **`r as.character(SummarizedExperiment::metadata(ribo_to_check)[["cscore_method"]])`** <br>
Window length : **`r as.character(SummarizedExperiment::metadata(ribo_to_check)[["cscore_window"]])`**<br>
...
**`r ifelse(isTRUE(SummarizedExperiment::metadata(ribo_to_check)[["combatSeq_count"]]),"Yes","No")`** <br>
Name of column in metadata used for ComBat-seq adjustment:
**`r ifelse(is.null(SummarizedExperiment::metadata(ribo_to_check)[["col_used_combatSeq"]]),"None",as.character(SummarizedExperiment::metadata(ribo_to_check)[["col_used_combatSeq"]]))`**
...
This QC report is generated from the analysis of the project **`r params$project_name`**, which is loaded inside the SummarizedExperiment object **`r params$ribo_name`**. This **`r params$ribo_name`** object was generated from **`r ncol(ribo_to_check)`** samples and **`r as.character(all_sites)`** genomic positions.

The following (r)RNA were used by the coverage tool: **`r as.character(SummarizedExperiment::metadata(ribo_to_check)[["rna_names"]][["original_name"]])`**

## Method

**C-score computation** : In the following analyses, the C-Score was computed using the **`r as.character(SummarizedExperiment::metadata(ribo_to_check)[["cscore_method"]])`** of the **`r as.character(SummarizedExperiment::metadata(ribo_to_check)[["cscore_window"]])`** neighboring positions (Birkedal et al, 2015; Marcel et al, 2020). In the end, there are **`r as.character(valid_sites)`** genomic positions with a valid C-score in all samples. Of note, C-score for genomic positions on RNA’s extremities cannot be computed because of the lack of neighbors in the local window.

**Sequencing biases correction** : The possible technical biases induced by the sequencing step are corrected using the **ComBat-seq** method (Zhang et al, 2020; Paraqindes et al, 2023). In this analysis, the correction is **`r ifelse(isTRUE(SummarizedExperiment::metadata(ribo_to_check)[["combatSeq_count"]]),"used","unused")`**. Here we used the variable "**`r ifelse(is.null(SummarizedExperiment::metadata(ribo_to_check)[["col_used_combatSeq"]]),"None",as.character(SummarizedExperiment::metadata(ribo_to_check)[["col_used_combatSeq"]]))`**" to perform this correction.
...
```{r, echo=FALSE}
knitr::kable(SummarizedExperiment::colData(ribo_to_check))
```
...
rRMSAnalyzer::plot_boxplot_count(ribo_to_check) + ggplot2::ggtitle("End read counts distribution by sample")
```
