---
title: "Comprehensive Guide to RiboMethSeq Analysis with rRMSAnalyzer"
author: "Théo COMBE, Hermes PARAQINDES, Janisse KIELBASSA, Emilie THOMAS, Anthony FERRARI and Virginie MARCEL"
date: "`r format(Sys.Date(), '%m/%d/%Y')`"
abstract: > 
  **rRMSAnalyzer** is a comprehensive R package designed for the analysis and visualization of **rRNA 2'-O-ribose methylation (2'-O-Me)**, a vital chemical modification that fine-tunes ribosomal function and translation accuracy. Derived from RiboMethSeq data, the package allows for the precise quantification of 2'-O-Me levels through the computation of **C-scores**. 
  
  This vignette provides a step-by-step guide to using rRMSAnalyzer, covering:
  
  *   Data importation and creation of `SummarizedExperiment` objects.
  *   Computation of C-scores using flexible parameters.
  *   Quality control and batch effect correction using ComBat-Seq.
  *   Visualization of methylation landscapes (PCA, Heatmaps).
  *   Differential methylation analysis.
  *   Annotation of known human rRNA methylation sites.
  
  The package is built on the `SummarizedExperiment` class, ensuring seamless integration with the Bioconductor ecosystem and robust data management.
output:
   rmarkdown::html_vignette:
      ...
      fig_height: 5
vignette: >
  %\VignetteIndexEntry{Comprehensive Guide to RiboMethSeq Analysis with rRMSAnalyzer}
...
Ribosomal RNA (rRNA) 2'-O-ribose methylation (2'-O-Me) is a conserved chemical modification that plays a crucial role in ribosome biogenesis and function. RiboMethSeq is a powerful RNA-seq-based approach for mapping and quantifying these modifications with single-nucleotide resolution.

**rRMSAnalyzer** simplifies the analysis of RiboMethSeq data by providing a suite of tools for:

*   **Quantification**: calculating C-scores from read-end counts.
*   **Quality Control**: assessing data quality and identifying technical biases.
*   **Correction**: adjusting for batch effects to ensure robust biological comparisons.
*   **Visualization**: generating high-quality plots to explore methylation patterns.
...
The \code{SummarizedExperiment} is the main class of the package that enables the storage of both the data matrices (counts and c-scores) and the associated metadata. It is automatically created when calling *load_ribodata* (see [Loading data]).

It consists of several components, individually described below:

1.  **Assays**: matrices containing for each sample the counts provided by the user, and the calculated C-score.

2.  **ColData**: a dataframe containing all the information related to the samples (metadata).

3.  **RowData**: a dataframe reporting the names of the RNA, positions and annotations for each site.

4. **Metadata**: a list containing additional information, such as the RNA names mapping table and computation parameters.
...
To load both data and metadata, and store them in a SummarizedExperiment, the function `load_ribodata` is used.
...
RNA names are automatically obtained from the data and stored in a generated dataframe inside the SummarizedExperiment object's metadata. It contains two columns :
...
During the analysis, the C-score calculating parameters (method and size of the flanking region) can be modified using the *compute_cscore* function, which will automatically update the C-score in the SummarizedExperiment.
...
**Important: this function will override the previous c-score of the SummarizedExperiment.**
...
- End read counts distribution by sample (plot_boxplot_count)
...
A sample subset can be easily analyzed by specifying which samples to keep or which to remove. The user can then create a new SummarizedExperiment object containing the data and metadata of the samples of interest. In both cases, only the metadata of the remaining samples are kept in the SummarizedExperiment object, so no manual updating is required.

Here is an example of how to create a new SummarizedExperiment by retaining two samples of interest (“S1” and “S2”):

```{r}
ribo_2samples <- keep_samples(ribo_adjusted, c("S1", "S2"))
print(ribo_2samples)
```

Here is an example to generate a new SummarizedExperiment by removing two samples ("S1" and "S2"):

```{r}
ribo_removed_samples <- remove_samples(ribo, c("S1", "S1"))
print(ribo_removed_samples)
```

In both cases, only the remaining samples' metadata are kept in the SummarizedExperiment object. There is no need to update it manually.
...
A subset of RNA can be easily analyzed by specifying the RNA to be removed. The user can thus create a new SummarizedExperiment object containing the data of the RNAs of interest, without affecting the metadata of the samples.
...
Here is an example to check if the RNA names provided by the user in the SummarizedExperiment match the ones used by this package :

```{r}
data("human_methylated")
cat("human_methylated's rna names: ", unique(human_methylated$rRNA), "\n")
cat("ribo's rna names: ", as.character(metadata(ribo_adjusted)$rna_names$current_name))
```
...
# from the shortest RNA in our SummarizedExperiment to the longest.
```
...
Instead of using the list of human rRNA 2’Ome sites provided by the rRMSAnalyzer package, the user can provide their own list of annotated sites which will be attached to the SummarizedExperiment object using the annotate_site function (see 10.3 section : annotation of RNA sites).
...
-   RNA name: the name of the RNA, matching the RNA name of the SummarizedExperiment.
...
The 2’Ome sites of interest must be attached to the SummarizedExperiment object for further analysis using the annotate function with either the provided annotations or custom annotations.
...
Note : rRMSAnalyzer will display a warning message when there is a mismatch between the annot and SummarizedExperiment RNA names. 
...
- boxplot using the C-scores of the annotated RNA 2’Ome sites in ascending order of variability (plot_boxplot_count)
...
# Visualizing 2'-O-Methylation on the Ribosome Structure

Understanding the spatial distribution of chemical modifications is key to unraveling their function. **rRMSAnalyzer** leverages the power of `r3dmol` to provide interactive 3D visualizations of the ribosome, mapping your methylation data directly onto the molecular structure.

The `plot_3d_structure` function fetches the 3D structure (e.g., human ribosome PDB: 4UG0), highlights annotated methylation sites, and allows for interactive exploration.

```{r, eval=FALSE}
# Visualize the 3D structure of the ribosome with methylation sites highlighted
# Note: This will generate an interactive 3D plot in your viewer or HTML report
plot_3d_structure(ribo_adjusted,
  pdb_id = "4UG0",
  site_color = "red",
  sphere_radius = 5,
  rna_color = "spectrum",
  show_proteins = FALSE # Hide proteins for better performance
)
```

> **Interactive Visualization**: In the generated HTML report, you can rotate, zoom, and inspect the ribosome structure to see exactly where your modifications are located relative to functional centers.
